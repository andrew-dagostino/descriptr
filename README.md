...
Unit tests can be run with the following command while in the backend directory:
...
```
pytest --cov=backend/classes/ --cov=backend/functions/ backend/test/
```
...
1. To run electron application, you must first run the command from the root directory:
